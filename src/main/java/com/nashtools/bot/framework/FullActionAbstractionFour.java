...
public class FullActionAbstractionFour extends ActionAbstraction {
	
	public final static double[] initial_betsizes = new double[]{0.25, 0.4, 0.5, 0.6, 0.75, 1, 1.5, 2, 3};
	public final static double[] raise_betsizes = new double[]{0.25, 0.5, 0.75, 1, 1.5, 2, 3};
	public final static double[] fourbet_betsizes = new double[]{1, 1.5, 2, 3};
	...
	public int get_actions(Game game, State state, Action[] actions) {
		  ...
		  if(round == 0){
			  if(num_bets<=2 || (num_bets==3 && state.action[round][0].type.ordinal()==1)){
				  ...
				  for(int i = 0; i < fourbet_betsizes.length; ++i){		  
					  int size = (int)(pot * fourbet_betsizes[i] + ( state.spent[ player ] + amount_to_call ));
	                  if( size < pre_commitment && size  >= min_raise_size[0]) {
		                    actions[ num_actions ] = action.copy();
		                    actions[ num_actions ].size = size ;
		                    ++num_actions;
			  ...
			  if(num_bets==0 || (num_bets==1 && state.action[round][0].type.ordinal()==1)){
			  ...
			  else if(num_bets<=2 || (num_bets==3 && state.action[round][0].type.ordinal()==1)){
				  for(int i = 0; i < raise_betsizes.length; ++i){		  
					  int size = (int)(pot * raise_betsizes[i] + ( state.spent[ player ] + amount_to_call ));
	                  if( size < post_commitment && size  >= min_raise_size[0]) {
		                    actions[ num_actions ] = action.copy();
