...
/**
 * Non-instantiable class containing a variety of static poker hand evaluation
 * and related utility methods.
 * <p>
 * All of the methods are thread-safe.
 * <p>
 * Each evaluation method takes a single parameter representing a hand of five
 * to seven cards represented in four 13-bit masks, one mask per suit, in the
 * low-order 52 bits of a long (64 bits). In each mask, bit 0 set (0x0001) for a
 * deuce, ..., bit 12 set (0x1000) for an ace. Each mask denotes the ranks
 * present in one of the suits. The ordering of the 13-bit suit fields is
 * immaterial.
 * <p>
 ...
 * <p>
 * Different methods are called for high and for lowball evaluation. The return
 * value format below is the same except:&nbsp;&nbsp;For low evaluation, as for
 * wheels in high evaluation, Ace is rank 1 and its mask bit is the LS bit;
 * otherwise Ace is rank 14, mask bit 0x1000, and the deuce's mask bit is the LS
 * bit. 8-or-better low evaluation methods may also return {@link #NO_8_LOW}.
 * <p>
 ...
 * <p>
 * Each evaluation method's return value is an int; 32 bits = 0x0VTBKKKK where
 * each letter refers to a 4-bit nybble:
 * 
 * <pre>
...
public final class HandEval {
	 ...
	 *         values to build a parameter to one of the HandEval evaluation
	 *         methods.
	 */
	...
	public static enum HandCategory {
		NO_PAIR, PAIR, TWO_PAIR, THREE_OF_A_KIND, STRAIGHT, FLUSH, FULL_HOUSE, FOUR_OF_A_KIND,
		STRAIGHT_FLUSH;
	}
	...
	// javac doesn't propagate NO_PAIR (==0) so doesn't constant-fold it out of
	// bitwise-or expressions
	// private static final int NO_PAIR = HandCategory.NO_PAIR.ordinal() <<
	// VALUE_SHIFT;
	...
	public static final int ARRAY_SIZE = 0x1FC0 + 1; // all combos of up to 7 of
																										// LS 13 bits on

	private static final int[] straightValue = new int[ARRAY_SIZE]; // STRAIGHT |
																																	// (straight's
																																	// high card
																																	// rank
																																	// (5..14) <<
																																	...
																																	// 0 if no
																																	// straight
	private static final int[] nbrOfRanks = new int[ARRAY_SIZE]; // count of bits
																																// set
	private static final int[] hiTopRankTWO_PAIR = new int[ARRAY_SIZE]; // TWO_PAIR
																																			// |
																																			// ((rank
																																			// (2..kA)
																																			// of the
																																			...
																																			// set) <<
																																			// TOP_SHIFT)
	private static final int[] hiBotRank = new int[ARRAY_SIZE]; // (rank (2..kA)
																															// of the highest
																															// bit set) <<
																															// BOT_SHIFT
	private static final int[] hiRankMask = new int[ARRAY_SIZE]; // all bits
																																// except
																																// highest reset
	private static final int[] hi2RanksMask = new int[ARRAY_SIZE]; // all bits
																																	// except
																																	// highest 2
																																	// reset
	private static final int[] hi3RanksMask = new int[ARRAY_SIZE]; // all bits
																																	// except
																																	// highest 3
																																	// reset
	private static final int[] hi5RanksMask = new int[ARRAY_SIZE]; // all bits
																																	// except
																																	// highest 5
																																	// reset

	private static final int[] lo5RanksMask = new int[ARRAY_SIZE]; // all bits
																																	// except
																																	// lowest 5
																																	// 8-or-better
																																	// reset; 0 if
																																	...
																																	// 8-or-better
																																	// bits set
	private static final int[] lo3RanksMask = new int[ARRAY_SIZE]; // all bits
																																	// except
																																	// lowest 3
																																	// 8-or-better
																																	// reset; 0 if
	...
	private static final int[] loEvalOrNo8Low = new int[ARRAY_SIZE]; // 5 bits set
																																		// in LS 8
																																		// bits, or
																																		// NO_8_LOW
																																		// */

	private static int flushAndOrStraight7(final int ranks, final int c, final int d, final int h,
			final int s) {
	...
	private static int flushAndOrStraight6(final int ranks, final int c, final int d, final int h,
			final int s) {
	...
	/**
	 * Returns the bitwise OR of the suit masks comprising <code>hand</code>; Ace
	 * is high.
	 * 
	 * @param hand
	...
	/**
	 * Returns the 8-or-better low value of a 5-card poker hand or
	 * {@link #NO_8_LOW}.
	 * 
	 * @param hand
	...
	/**
	 * Returns the 8-or-better low value of the best hand from hole cards and 3
	 * board cards or {@link #NO_8_LOW}.
	 * 
	 * @param holeRanks
	 *          bit mask of the rank(s) of hole cards (Ace is LS bit).
	 * @param boardRanks
	 *          bit mask of the rank(s) of board cards (Ace is LS bit).
	 * @return the 8-or-better low value of the best hand from hole cards and 3
	 *         board cards or {@link #NO_8_LOW}.
	 * @see #ranksMaskLo
	 */
	...
	public static void main(String[] args) {
		...
		Card c2 = new Card(Card.Rank.KING, Card.Suit.CLUB);
		Card c3 = new Card(Card.Rank.QUEEN, Card.Suit.CLUB);
		Card c4 = new Card(Card.Rank.JACK, Card.Suit.CLUB);
		Card c5 = new Card(Card.Rank.TEN, Card.Suit.CLUB);
		Card c6 = new Card(Card.Rank.TWO, Card.Suit.SPADE);
		Card c7 = new Card(Card.Rank.THREE, Card.Suit.SPADE);
		Card[] cards = {c1, c2, c3, c4, c5, c6, c7};
		int res = hand7Eval(encode(cards));
		int type = res >> 24;
