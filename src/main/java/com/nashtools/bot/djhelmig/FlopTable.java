...
public class FlopTable {
	 ...
	 */
	private float lookupOne(int cards[]) {
		...
		// Table.printArray("suitPattern", suitPattern);
	...
	private FlopEHS fthread[] = new FlopEHS[2];
	...
	private void fillTable(int[] Rank, int suitpattern[], int rankIsoIndex, int suitIndex) {
		...
		if (count % 1000 == 0) {
			// System.out.println("Finished " + count + " ranksEnumerated" +
			// ranksEnumerated);
	...
	public void rankSuitsDebug() {
		...
		int rankIsoIndex = rankIndexMap[rankidx];
		System.out.println("rankIsoIndex:" + rankIsoIndex);

		for (int i = 0; i < numRankPattern.length; i++)
			System.out.println("numRankPattern[" + i + "]=" + numRankPattern[i]);
		...
		for (int i = 0; i < f.getSize(); i++) {
			...
			float ehs = FlopEHS.flopklaatuEHS(cards);

			Table.printArray("EHS:" + ehs, suits);
	...
	public void handDebug() {
		...
		float f = FlopEHS.flopklaatuEHS(h);
		float f2 = FlopEHS.flopklaatuEHS(h2);
		float f3 = FlopEHS.flopklaatuEHS(h3);
		float f4 = FlopEHS.flopklaatuEHS(h4);
	...
	public int testLUT(int n) {
		...
		for (int i = 0; i < n; i++) {
			...
			float direct = FlopEHS.flopklaatuEHS(cards);
	...
	public static void main(String[] args) {
		...
		f.initialize();
		float r = f.lookupOne(cards);
		System.out.println(f.LUT[0]);
		System.out.println(f.LUT[1]);
		System.out.println(f.LUT[2]);
		// f.rankSuitsDebug();

		int ntested = f.testLUT(10);
