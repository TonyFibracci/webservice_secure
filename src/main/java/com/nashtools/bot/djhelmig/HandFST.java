...
import java.io.*;
import java.util.*;
// We use Steve Brecher's HandEval during FST generation, although any correct
// hand evaluator would work as well
// import com.stevebrecher.poker.HandEval;

/**
 * FST (Finite State Transducer) generator helper class for the {@code FSTEval}
 * poker hand evaluation methods.
 * <p>
 * This class generates the finite state transducer arrays used by
 * {@code FSTEval} to evaluate poker hands. The evaulation arrays and associated
 * utility map data are serialized and stored in files. When a request is made
 * for the arrays, an attempt is made to read them from the serialized files. If
 * the files are not found, the arrays are regenerated.
 * <p>
 * Input to the FST are the poker cards that form the hand. In order to keep the
 * FST at a manageable size, only the ranks of the cards are considered. The
 * initial state of the FST represents a zero-card hand. Each dealt card causes
 * a transition to another valid FST state (there are no invalid inputs in a
 * poker hand). Because card order does not affect the value of the hand, states
 * with different card permutations are coalesced. States with identical
 * transistions for every input card (e.g. the states for the six-card hands [2
 * 2 2 3 5 5] and [2 2 2 4 5 5] in a seven-card game) are also coalesced. The
 * final FST states are the equivalence class numbers of the best 5-card high
 * poker hand that can be made from the input cards (5, 6, and 7-card games are
 * supported).
 * <p>
 * The FST states are compactly written as {@code char} arrays (Java's
 * {@code unsigned short} type). For an n-card game, Each array entry in the
 * first through n-1 arrays, when added to the next card rank value, gives the
 * index of the entry into the next-level array. The entries in the last array
 * are the equivalence class values ([0..4823] for a 7-card game, [0..6074] for
 * a 6-card game and [0..7461] for a 5-card game).
 * <p>
 * A separate flush array is also generated. For each encoded {@code int} bit
 * vector array index of the 5-n suited cards of the hand, the array entry at
 * that index is the equivalence class value of the flush or straight flush
 * hand.
 * <p>
 * As an optimization, a separate array is written that combines the rank values
 * of the first three cards. That is, for each index value
 * {@code ((((rankOf(card1)) << 8) | (rankOf(card2) << 4)) | rankOf(card3))},
 * the array entry at that index contains a value that may be used to index into
 * the fourth-card array.
 * <p>
 * For seven card hands, the five evaluation arrays (four single card arrays and
 * the first three-card array) together with the flush array total 641350 bytes
 * in memory.
 * <p>
 * For six card hands, the five evaluation arrays (four single card arrays and
 * the first three-card array) together with the flush array total 242614 bytes
 * in memory.
 * <p>
 * For five card hands, the five evaluation arrays (four single card arrays and
 * the first three-card array) together with the flush array total 82064 bytes
 * in memory.
 * <p>
 * The generation routines use the {@code com.stevebrecher.poker.HandEval}
 * evaulation routines to generate the equivalence class data, although any
 * correct hand evaluator would suffice. An array and HashMap are also generated
 * that convert equivalence class numbers to {@code HandEval} return values and
 * vice versa.
 * <p>
 * After being generated, the arrays and map data are serialized and written to
 * the files HandFST&ltn&gt.ser which are by default located in the current
 * directory. The directory of the HandFST&ltn&gt.ser files may be changed by
 * invoking the method {@code setDirectory} prior to being used.
 * <p>
 * 
 * @author Klaatu
 */
...
public class HandFST {
	...
	private static final int WIDE_RANK_SIZE = 16; // next power of 2 above
																								// RANK_SIZE

	/**
	 * Controls whether "wide arrays" are used when the lookup value overflows
	 * Character.MAX_VALUE.
	 * When this overflow occurs the lookup value cannot be pre-multiplied by the
	 * rank count (13).
	 * If OPT_WIDE_RANK_SIZE is set to the next power of two (16), these array
	 * sizes
	 * are rounded up so that the eval routines can use shift instructions instead
	 * of multiplications.
	 * In the 7-card FST, this adds an additional ~125K of memory to the arrays
	 * but allows the eval7 routine
	 * to replace two multiplication instructions with shift instructions.
	 * <p>
	 * Using wide arrays speeds up hand enumerations by around 5%, but slows down
	 * random evaluations due to the extra memory required. It's a close call, but
	 * they're turned off by default for now. If you would like to turn them on,
	 * set the value of OPT_WIDE_RANK_SIZE to be WIDE_RANK_SIZE, and recompile
	 * {@code FSTEval.java} and {@code HandFST.java}.
	 * <p>
	 * We continue to use a "wide" index for the (small) card123 jump start array,
	 * however, since the additional memory cost is negligible.
	 */
	...
	private int[] _equivalentStates; // set by minimize(), used to coalesce
																		// equivalent states
	...
	/**
	 * Sets the current default directory in which to store serialized FST data.
	 * The
	 * default directory is used by the {@code HandFST} constructors that do not
	 * take
	 * a filename argument.
	 * 
	 * @param dir
	 *          the current default directory in which to store serialized FST
	 *          data.
	 */
	...
	/**
	 * Returns the current default directory in which to store serialized FST
	 * data.
	 * 
	 * @return the current default directory in which to store serialized FST
	 *         data.
	 */
	...
	private boolean haveArrays() {
		...
		return _jumpStartArray != null && _levelArrays != null && _flushArray != null &&
				_toBrecherArray != null && _toBrecherArray != null && _fromBrecherMap != null;
	}
	...
	private void ensureArrays(int level) {
		...
		if (level > _handLength)
			throw new IllegalStateException("Illegal to request card " + level + " arrays from an " +
					_handLength + " FST");
		try {
			if (!haveArrays())
				readArrays();
			...
			System.out.println("IO Exception + e + reading FST array file " + _filename +
					", rebuilding...");
		} catch (ClassNotFoundException e) {
			// not really possible
		} catch (IllegalStateException e) {
			System.out.println("FST array file " + _filename +
					" built with different OPT_WIDE_RANK_SIZE, rebuilding...");
		}
		try {
			if (!haveArrays())
				rebuild(true);
		...
		if (!haveArrays())
			throw new IllegalStateException("Unable to find or rebuild FST arrays for " + _handLength +
					"-card hands");
	}

	/*
	 * This class represents a state in the FST that occurs after the array of
	 * _cards
	...
	private static int makeHand(int[] ranks) {
		...
		return ranks.length == 7 ? HandEval.hand7Eval(bval) : ranks.length == 6 ? HandEval
				.hand6Eval(bval) : HandEval.hand5Eval(bval);
	}
	...
	private static int makeFlushHand(int[] ranks) {
		...
		return ranks.length == 7 ? HandEval.hand7Eval(bfval) : ranks.length == 6 ? HandEval
				.hand6Eval(bfval) : HandEval.hand5Eval(bfval);
	}
	...
	// Rebuild the FST as needed (when the file containing the FST array data is
	// not found)
	private void rebuild(boolean recalculate) {
		...
		try {
			...
			System.out.println("Unable to write FST to " + _filename + ": " + e +
	...
	private void calculate() {
		...
		if (_handLength == 7)
			System.out
					.println("Calculating 7-card FST (this is done one time only and will take a minute or so) ...");
		else
	...
	// Make the states of the FST for all the possible card rank values of the
	// hand
	...
	// Make the final flush states. These aren't part of the FST (there are no
	// transistions
	...
	private void minimize() {
		...
		for (int i = 0; i < numStates; i++) {
			// Place each final state in its own partition, since we want more than a
			// yes/no answer
		...
		for (int bn = 0; bn < partitionCount; bn++) {
			...
			while (it.hasNext()) {
				...
				if (stateNum == numStates)
					continue;
				if (DEBUG)
					System.out.println("Coalescing state " + _allStates[stateNum] + " with " +
							_allStates[eqvnum]);
				...
				if (nfhs._transistions != null) {
					for (int j = 0; j < nfhs._transistions.length; j++) {
						...
						if (trans != null) {
							...
							if (_equivalentStates != null && _equivalentStates[txidx] != -1 &&
	...
	// Split the states into separate arrays, one for each number of cards. This
	// is
	// possible because we never coalesce states with different card numbers. The
	// FST is
	// ultimately stored as a set of arrays, one per level, to make array index
	// sizes
	...
	private void buildLevels() {
		...
		for (int i = 0; i < _allStates.length; i++) {
			...
			if (hs._cards != null && _equivalentStates[hs._index] == hs._index) {
				// int level = hs._transistions == null ? _handLength :
				// hs._cards.length;
	...
	// Order all of the final states. Once ordered, the index into the sorted
	// array
	// is the eqivalence class of the hand (which is the result of the eval
	// methods).
	...
	// arrays as small as possible by using the smallest array type in which all
	// entries will
	...
	// Build the arrays for a single level. Non-final level arrays contain indexes
	// into
	// the next level arrays, pre-multiplied by RANK_SIZE if the index will fit in
	// a char.
	...
	private char[] buildLevelArray(int n) {
		...
		boolean preMultiply = !useEqvClass &&
				_levelStates[n + 1].length * RANK_SIZE <= Character.MAX_VALUE;
	...
	// Construct a separate array that holds all of the possible flush and
	// straight flush
	// hand equivalence class values. The index into the array is a bit vector
	// that
	...
	// Serialize the constructed eval arrays and utility maps and write them into
	// a file
	...
	// Read the serialize the constructed eval arrays and utility maps from a
	// file.
