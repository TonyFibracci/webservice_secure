 ...
 * EXACT TODO:
 * APPROX TODO:
 * If blocking only 1 suit it matters very little which one (try 0.25707385 vs.
 * 0.25704312) for general
 * rank patterns.
 */

public class FlopSuit {
	 ...
	 * with respect to this rank pattern. Only used during enumeration, not
	 * lookup.
	 */
	...
	FlopSuit() {
		...
		int sizev3[] = new int[] { 4, 4, 4 };

		// just set all suitMap entries to -1
		Table.init_int5(suitMap, sizev, -1);
	...
	private void addSuit(int Rank[], int suits[]) {
		...
		int isuit[] = SuitIso.lowestSuit(suits);
